...
1. **合并优于追加**：如果目标位置已有相关内容，修改现有条目而不是在末尾追加新条目。
2. **删除优于保留**：标记为"过期"或"被推翻"的信息，直接删除，不要留"已废弃"注释。
3. **绝对时间**：所有时间用 `YYYY-MM-DD` 格式，不写"今天"、"最近"。
4. **一条一事**：每个记忆条目只说一件事。
5. **面向读者**：写的时候想象下一个读到这条的 agent 只有 5 秒能扫过。
...
只列有实际变更的项。没动的不写。遇到无法自动判断的矛盾，列在"需用户确认"。
